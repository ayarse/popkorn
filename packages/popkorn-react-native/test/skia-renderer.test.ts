...
import { parse, buildSceneGraph, RenderLoop, multiplyMatrices, IDENTITY_MATRIX } from '@popkorn/player';
...
// PopkornView does after delivering the resting frame) the renderer records no
...
// #2: a paused (dynamic) loop freezes the timeline, and the dormancy PopkornView
...
// (PopkornView's frameCallback is the integrator; it can't mount headless.)
...
// hasPendingImages(): the seam PopkornView polls after a settle/freeze to
// decide whether to schedule a wake-up for a still-decoding file://http(s)
// image (see PopkornView's `wakeWhenImagesSettle`). data: URIs decode
