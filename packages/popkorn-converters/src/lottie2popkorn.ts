 ...
 * because Lottie paints last-to-first and Popkorn paints first-behind). Layer
...
export function prop(p: any): Prop | null {
  ...
  if (p.s === true && (p.x !== undefined || p.y !== undefined))
    return splitProp(p);
...
export function axesDiverge(x: Prop, y: Prop): boolean {
  ...
  if (xk.length !== yk.length || xk.some((k, i) => k.t !== yk[i].t))
    return true;
  const ez = (k: Kf) =>
    k.h === 1
      ? "h"
      : k.o && k.i
        ? `${first(k.o.x)},${first(k.o.y)},${first(k.i.x)},${first(k.i.y)}`
        : "";
...
function sampleStopScalar<T extends { pos: number }>(
  stops: T[],
  p: number,
  get: (s: T) => number,
): number {
...
export function normalizeDoc(lottie: any): void {
  ...
  if (Array.isArray(lottie.assets))
    for (const a of lottie.assets)
      if (a && Array.isArray(a.layers)) fixInds(a.layers);
}
...
export class Converter {
  ...
  private dedupeImages(): string[] {
    ...
    for (const u of this.imageUses)
      count.set(u.assetId, (count.get(u.assetId) || 0) + 1);
    const vars: string[] = [];
  ...
  private buildLayerList(
    ...
    const prevClampIp = this.clampIp,
      prevClampOp = this.clampOp;
    this.clampIp = compIp;
    this.clampOp = compOp;
    ...
    for (const l of layers) {
      ...
      for (const e of l.ef) {
        ...
        const name =
          typeof e.nm === "string" && e.nm.trim()
            ? e.nm.trim()
            : `type ${e.ty}`;
        this.warnOnce(
          `layer effect '${name}' is not supported and was ignored`,
        );
      }
    }
    ...
    const isOrphanMatte = (l: any) =>
      !!l && !!l.td && typeof l.ind === "number" && !consumedSrc.has(l.ind);
    ...
    for (const l of layers) {
      ...
      if (isOrphanMatte(l)) {
        this.warnOnce(
          `orphan track-matte source '${l.nm ?? l.ind}' dropped (no tt consumer)`,
        );
        continue;
      }
      ...
      if (
        ...
        this.isConvertible(byInd.get(parent)) &&
        !isOrphanMatte(byInd.get(parent))
      ) {
        (
        ...
        if (typeof parent === "number")
          this.warnOnce(
            `layer ${l.ind} references missing parent ${parent}; treated as unparented`,
          );
        roots.push(l);
      }
    }
    ...
    // Lottie paints last-to-first; Popkorn paints first-behind -> reverse.
    const topRules: Rule[] = [];
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      if (!mode) {
        this.blocked.add(`track mask mode tt:${l.tt}`);
        continue;
      }
      ...
      if (!content || !source) {
        this.blocked.add("track mask (unresolved source)");
        continue;
      }
  ...
  private effectFilterDecl(l: any): string | null {
    ...
    for (const e of l.ef) {
      ...
      if (e.ty === 29) {
        const sub =
          (e.ef || []).find((s: any) => s && s.nm === "Blurriness") ||
          (e.ef || [])[0];
        const p = sub && prop(sub.v);
        if (!p) continue;
        if (p.animated)
          this.warnOnce(
            `animated Gaussian Blur on '${name}' baked to its first value`,
          );
        const px = (p.at(0)[0] ?? 0) / 4;
        if (px > 0) parts.push(`blur(${num(px)}px)`);
      } else if (e.ty === 25) {
        const sub = e.ef || [];
        const cp = prop(sub[0]?.v),
          op = prop(sub[1]?.v),
          dir = prop(sub[2]?.v),
          dist = prop(sub[3]?.v),
          soft = prop(sub[4]?.v);
        if ([cp, op, dir, dist, soft].some((x) => x && x.animated))
          this.warnOnce(
            `animated Drop Shadow on '${name}' baked to its first value`,
          );
        ...
        parts.push(
          `drop-shadow(${num(distance * Math.cos(ang))}px ${num(distance * Math.sin(ang))}px ${num(blur)}px ${color})`,
        );
      }
    }
  ...
  private buildLayerRule(l: any, ctx: LayerCtx): Rule {
    ...
    if (l.ty === 0) {
      ...
      if (mask) this.applyMask(content, mask);
        ...
        content.decls.push(
          `clip-path: path('M0 0 H${num(cw)} V${num(ch)} H0 Z')`,
        );
    ...
    if (l.ty === 2) {
      ...
      const asset = this.assets.get(l.refId);
      if (!asset) {
        this.blocked.add("image layer missing asset");
        throw new Error(`missing asset '${l.refId}'`);
      }
      const img: Rule = {
        id,
        type: "image",
   ...
   */
  /**
   * Serialize a string as a Popkorn string literal. The parser's readString does
   ...
   * unioned. Modelled on lottie-web's *canvas* renderer (Popkorn's target),
   ...
   * clip morphs with the mask (Popkorn animates the path() command list, exactly
  ...
  private maskClip(l: any): { base: string; channel: Channel | null } | null {
    ...
    for (const m of masks) {
      ...
      if (m.inv) {
        this.blocked.add("inverted mask (inv)");
        return null;
      }
      const o = prop(m.o);
      if (o && (o.animated || (o.at(0)[0] ?? 100) < 100)) {
        this.blocked.add("mask opacity < 100");
        return null;
      }
      const pt = m.pt;
      if (!pt) {
        this.blocked.add("mask missing shape");
        return null;
      }
      const x = prop(m.x);
      if (x && Math.abs(x.at(0)[0] ?? 0) > 1e-6)
        this.warnOnce(`mask expansion on '${l.nm || l.ind}' ignored`);
      if (isAnimatedShape(pt)) {
        const kfs = pathKfs({ ks: pt });
        animKfs.push(...kfs);
        samplers.push((t) => {
          const d = shapeToPath(shapeAt(kfs, t));
          return d ? `path('${d}')` : "";
        });
      } else {
  ...
  private applyMask(
    rule: Rule,
    mask: { base: string; channel: Channel | null },
  ) {
  ...
  private applyTransform(
    ...
    if (!positionHandled && ks.p && ks.p.s === true) {
      const px = prop(ks.p.x),
        py = prop(ks.p.y);
      if (px && py && px.animated && py.animated && axesDiverge(px, py)) {
        rule.channels.push({
          priority: 5,
          kfs: px.kfs!,
          sample: (t) => ({ txx: (px.at(t)[0] || 0) - ax }),
        });
        rule.channels.push({
          priority: 5,
          kfs: py.kfs!,
          sample: (t) => ({ txy: (py.at(t)[0] || 0) - ay }),
        });
        positionHandled = true;
      }
    }
  ...
  private applyMotionPath(
    ...
    for (let k = 0; k < verts.length - 1; k++) {
      ...
      const c1x = verts[k][0] + (to[0] || 0),
        c1y = verts[k][1] + (to[1] || 0);
      const c2x = verts[k + 1][0] + (ti[0] || 0),
        c2y = verts[k + 1][1] + (ti[1] || 0);
      d.push(
        `C ${num(c1x)} ${num(c1y)} ${num(c2x)} ${num(c2y)} ${num(verts[k + 1][0])} ${num(verts[k + 1][1])}`,
      );
    ...
    if (ax !== 0 || ay !== 0)
      rule.decls.push(`transform: translate(${num(-ax)}px, ${num(-ay)}px)`);
  ...
  private processItems(
      ...
      switch (it.ty) {
        ...
        case "st": {
          ...
          if (Array.isArray(it.d)) {
            ...
            for (const el of it.d) {
              ...
              if (v && v.animated)
                this.warnOnce("animated stroke dash baked to first value");
            }
        ...
        case "gs": {
          ...
          if (it.g && it.g.k && it.g.k.a === 1)
            this.warnOnce(
              "animated gradient stroke stops baked to first value",
            );
          if (gso && gso.animated)
            this.warnOnce(
              "animated gradient stroke opacity baked to first value",
            );
          break;
        }
        case "gf": {
          ...
          // fill channel of gradient() strings per keyframe (Popkorn interpolates
          ...
          if (gradTracks.length) {
            const times = [
              ...new Set(gradTracks.flatMap((k) => k.map((kf) => kf.t))),
            ].sort((a, b) => a - b);
            fillKfs = times.map((t) => {
              const src = gradTracks
                .map((tk) => tk.find((kf) => kf.t === t))
                .find(Boolean);
              return { t, i: src?.i, o: src?.o, h: src?.h };
            });
            fillCh = (t) => ({
              fill: this.gradientCssAt(it, t) ?? gradientFill!,
            });
          }
          break;
        }
          ...
          trim = {
            ...
            startCh:
              s && s.animated && s.kfs
                ? {
                    kfs: s.kfs,
                    sample: (t: number) => ({ trimStart: s!.at(t)[0] ?? 0 }),
                  }
                : null,
            endCh:
              e && e.animated && e.kfs
                ? {
                    kfs: e.kfs,
                    sample: (t: number) => ({ trimEnd: e!.at(t)[0] ?? 100 }),
                  }
                : null,
            offsetCh:
              o && o.animated && o.kfs
        ...
        default:
          if (it.ty in BLOCKED_MODIFIERS)
            this.blocked.add(BLOCKED_MODIFIERS[it.ty]);
      }
    }
    ...
    if (
      ...
      stroke &&
      strokeWidth != null &&
      items.some((it) => it.ty === "gr")
    ) {
      const covered = collectStrokeableDraws(items);
      if (covered && covered.length > 1) {
        const node = this.buildMergedPath(
          covered,
          this.uniqueId(prefix + "-stroke"),
          fillRule,
        );
        if (node) {
          node.decls.push(
            "fill: none",
            `stroke: ${stroke}`,
            `stroke-width: ${num(strokeWidth)}px`,
          );
          if (lineCap && lineCap !== "butt")
            node.decls.push(`stroke-linecap: ${lineCap}`);
          if (lineJoin && lineJoin !== "miter")
            node.decls.push(`stroke-linejoin: ${lineJoin}`);
          if (miterLimit != null && miterLimit !== 4)
            node.decls.push(`stroke-miterlimit: ${num(miterLimit)}`);
          if (dashArray)
            node.decls.push(
              `stroke-dasharray: ${dashArray.map((d) => `${num(d)}px`).join(" ")}`,
            );
          if (dashOffset)
            node.decls.push(`stroke-dashoffset: ${num(dashOffset)}px`);
          if (strokeCh && strokeKfs)
            node.channels.push({
              priority: 1,
              kfs: strokeKfs,
              sample: strokeCh,
    ...
    const applyStyle = (rule: Rule, layer?: (typeof fills)[number]) => {
      ...
      if (strokeWidth != null)
        rule.decls.push(`stroke-width: ${num(strokeWidth)}px`);
      if (lineCap && lineCap !== "butt")
        rule.decls.push(`stroke-linecap: ${lineCap}`);
      if (lineJoin && lineJoin !== "miter")
        rule.decls.push(`stroke-linejoin: ${lineJoin}`);
      if (miterLimit != null && miterLimit !== 4)
        rule.decls.push(`stroke-miterlimit: ${num(miterLimit)}`);
      if (dashArray)
        rule.decls.push(
          `stroke-dasharray: ${dashArray.map((d) => `${num(d)}px`).join(" ")}`,
        );
      if (dashOffset)
        rule.decls.push(`stroke-dashoffset: ${num(dashOffset)}px`);
      if ((layer ? layer.rule : fillRule) === 2)
        rule.decls.push(`fill-rule: evenodd`);
      if (effectiveTrim) {
        if (!effectiveTrim.startCh)
          rule.decls.push(`trim-start: ${num(effectiveTrim.start)}%`);
        if (!effectiveTrim.endCh)
          rule.decls.push(`trim-end: ${num(effectiveTrim.end)}%`);
        if (effectiveTrim.offset && !effectiveTrim.offsetCh)
          rule.decls.push(
            `trim-offset: ${num(effectiveTrim.offset * 100, 3)}%`,
          );
        if (effectiveTrim.startCh)
          rule.channels.push({
            priority: 1,
            kfs: effectiveTrim.startCh.kfs,
            sample: effectiveTrim.startCh.sample,
          });
        if (effectiveTrim.endCh)
          rule.channels.push({
            priority: 1,
            kfs: effectiveTrim.endCh.kfs,
            sample: effectiveTrim.endCh.sample,
          });
        if (effectiveTrim.offsetCh)
          rule.channels.push({
            priority: 1,
            kfs: effectiveTrim.offsetCh.kfs,
            sample: effectiveTrim.offsetCh.sample,
          });
      }
      const lch = layer ? layer.ch : fillCh,
        lkfs = layer ? layer.kfs : fillKfs;
      if (lch && lkfs)
        rule.channels.push({ priority: 1, kfs: lkfs, sample: lch });
      if (strokeCh && strokeKfs)
        rule.channels.push({ priority: 1, kfs: strokeKfs, sample: strokeCh });
    };
    ...
    if (mmIdx >= 0) {
      ...
      if (drawables.length > 1) {
        if (mode === 1 || mode === 2) {
          ...
          const merged = this.buildMergedPath(
            drawables,
            this.uniqueId(prefix + "-merge"),
            fillRule,
          );
          if (merged) {
            if (stroke)
              this.warnOnce(
                "stroke on a merged path shows interior seams (fills are unioned, strokes are not)",
              );
            applyStyle(merged);
    ...
    const paintLayers: ((typeof fills)[number] | undefined)[] =
      fills.length > 1 ? fills : [undefined];
    ...
    if (siblingDraws.length === 1) {
      ...
      emit(() =>
        this.buildMergedPath(
          siblingDraws,
          this.uniqueId(prefix + "-shapes"),
          fillRule,
        ),
      );
    }
    ...
    // Lottie paints shape items top-first; Popkorn paints first-behind -> reverse.
    return out.reverse();
  }
  ...
  private buildDrawable(it: any, rawId: string): Rule | null {
    ...
    if (it.ty === "el") {
      ...
      if (p && p.animated && p.kfs) {
        if (hasSpatialTangents(p.kfs))
          this.warnOnce("spatial tangents on a shape position ignored");
        rule.channels.push({
          priority: 5,
          kfs: p.kfs,
          sample: (t) => {
    ...
    if (it.ty === "sr") {
      ...
      if (or && or.animated && or.kfs)
        rule.channels.push({
          priority: 5,
          kfs: or.kfs,
          sample: (t) => ({ outerRadius: or.at(t)[0] }),
        });
      if (star && ir && ir.animated && ir.kfs)
        rule.channels.push({
          priority: 4,
          kfs: ir.kfs,
          sample: (t) => ({ innerRadius: ir.at(t)[0] }),
        });
      if (rot && rot.animated && rot.kfs)
        rule.channels.push({
          priority: 3,
          kfs: rot.kfs,
          sample: (t) => ({ starRotation: rot.at(t)[0] }),
        });
      if (p && p.animated && p.kfs)
        rule.channels.push({
          priority: 6,
    ...
    // channel — one path string per keyframe; Popkorn morphs between compatible
    ...
    if (isAnimatedShape(it.ks)) {
      ...
      rule.channels.push({
        priority: 6,
        kfs,
        sample: (t) => ({ d: shapeToPath(shapeAt(kfs, t)) }),
      });
      return rule;
    }
  ...
  private buildMergedPath(
    ...
    if (animated.length === 0) {
      ...
      const times = [
        ...new Set(tracks.flatMap((k) => k.map((kf) => kf.t))),
      ].sort((a, b) => a - b);
      const kfs: Kf[] = times.map((t) => {
        const src = tracks
          .map((tk) => tk.find((kf) => kf.t === t))
  ...
  private buildGradient(it: any, opacity = 1): string | null {
    ...
    // (Popkorn interpolates them; see registry 'gradient'). Static: bake at t0.
  ...
  private gradientStops(flat: number[], count: number, opacity = 1): string[] {
    ...
    for (let i = 0; i < count; i++) {
      colors.push({
        pos: flat[i * 4],
        rgb: [flat[i * 4 + 1], flat[i * 4 + 2], flat[i * 4 + 3]],
      });
    }
    const alphas: { pos: number; a: number }[] = [];
    for (let i = count * 4; i + 1 < flat.length; i += 2)
      alphas.push({ pos: flat[i], a: flat[i + 1] });
    if (alphas.length === 0)
      return colors.map(
        (c) => `${lottieColor(c.rgb, opacity)} ${num(c.pos * 100)}%`,
      );
    const positions = [
      ...new Set([...colors.map((c) => c.pos), ...alphas.map((a) => a.pos)]),
    ].sort((x, y) => x - y);
    return positions.map((p) => {
      const rgb = [0, 1, 2].map((k) =>
        sampleStopScalar(colors, p, (c) => c.rgb[k]),
      );
...
function declsFromSample(s: Sample): string[] {
  ...
  if (s.tx !== undefined || s.ty !== undefined)
    tf.push(`translate(${num(s.tx ?? 0)}px, ${num(s.ty ?? 0)}px)`);
  ...
  if (s.strokeWidth !== undefined)
    out.push(`stroke-width: ${num(s.strokeWidth)}px`);
  if (s.offsetDistance !== undefined)
    out.push(`offset-distance: ${num(s.offsetDistance * 100)}%`);
  if (s.trimStart !== undefined) out.push(`trim-start: ${num(s.trimStart)}%`);
  if (s.trimEnd !== undefined) out.push(`trim-end: ${num(s.trimEnd)}%`);
  if (s.trimOffset !== undefined)
    out.push(`trim-offset: ${num(s.trimOffset * 100, 3)}%`);
  if (s.outerRadius !== undefined)
    out.push(`outer-radius: ${num(s.outerRadius)}px`);
  if (s.innerRadius !== undefined)
    out.push(`inner-radius: ${num(s.innerRadius)}px`);
  if (s.starRotation !== undefined)
    out.push(`rotation: ${num(s.starRotation)}deg`);
...
/** Shape at frame t: hold the departing keyframe (Popkorn morphs between them). */
...
/** Lottie track-mask type (tt) -> Popkorn mask mode. */
const MASK_MODE: Record<number, string> = {
  1: "alpha",
  2: "alpha-invert",
  3: "luminance",
  4: "luminance-invert",
...
export function convertLottie(lottie: any): {
  css: string;
  warnings: string[];
  blocked: string[];
} {
