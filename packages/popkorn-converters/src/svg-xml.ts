...
function parseAttrs(c: Cursor, attrs: Map<string, string>): void {
  for (;;) {
    ...
    if (c.peek() === "=") {
      ...
      const quote = c.peek();
      if (quote !== '"' && quote !== "'")
        throw new Error(c.errorAt("expected quoted attribute value"));
      c.pos++; // open quote
      const end = c.src.indexOf(quote, c.pos);
      if (end === -1)
        throw new Error(c.errorAt("unterminated attribute value"));
...
function parseElement(c: Cursor): SvgNode {
  ...
  for (;;) {
    if (c.eof())
      throw new Error(
        c.errorAt(`unexpected end of input inside <${node.tag}>`),
      );
    ...
    if (c.starts("</")) {
      ...
      c.ws();
      if (c.peek() !== ">")
        throw new Error(c.errorAt("expected `>` on close tag"));
      c.pos++;
      if (localName(close) !== node.tag) {
        throw new Error(
          `mismatched close tag </${localName(close)}> for <${node.tag}> at offset ${c.pos}`,
        );
      }
      break;
    }
