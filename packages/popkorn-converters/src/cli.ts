 ...
 *
 *   bun packages/popkorn-converters/src/cli.ts <in.json|in.svg> [-o out.css] [--validate]
 *   bun packages/popkorn-converters/src/cli.ts --batch <dir> [--validate]   # walks by extension
 *
 * Node/Bun-only (fs, path, process) — kept out of the *2popkorn.ts cores so they
...
function main() {
  ...
  if (!input) {
    console.error(
      "usage: bun packages/popkorn-converters/src/cli.ts <in.json|in.svg> [-o out.css] [--validate]",
    );
    console.error(
      "       bun packages/popkorn-converters/src/cli.ts --batch <dir> [--validate]",
    );
    process.exit(1);
  }
