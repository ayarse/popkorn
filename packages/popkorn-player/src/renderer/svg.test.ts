...
test("pathToD: serializes the SVG path grammar 1:1", () => {
  ...
  expect(pathToD(cmds)).toBe(
    "M0 0L10 20C1 2 3 4 5 6Q7 8 9 10A5 5 0 1 0 12 13Z",
  );
});
...
test("maskModePlumbing: non-inverted modes pick mask-type, no filter", () => {
  expect(maskModePlumbing("alpha")).toEqual({
    maskType: "alpha",
    filter: null,
  });
  expect(maskModePlumbing("luminance")).toEqual({
    maskType: "luminance",
    filter: null,
  });
});
...
test("maskModePlumbing: inverted modes normalize to an alpha mask + flip filter", () => {
  ...
  expect(maskModePlumbing("alpha-invert")).toEqual({
    maskType: "alpha",
    filter: ["invertAlpha"],
  });
  expect(maskModePlumbing("luminance-invert")).toEqual({
    maskType: "alpha",
    filter: ["luminanceToAlpha", "invertAlpha"],
  });
});
...
test("compositeMask (alpha): builds an alpha <mask> and points the content wrapper at it", () => {
  ...
  expect([
    ...
    mask!.getAttribute("height"),
  ]).toEqual(["0", "0", "100", "100"]);

  // A wrapper <g> in the tree references the mask and holds the content node.
  const wrapper = findEl(
    rootG,
    (e) =>
      e.tagName === "g" &&
      e.getAttribute("mask") === `url(#${mask!.getAttribute("id")})`,
  );
...
test("compositeMask (luminance-invert): alpha mask + luminanceToAlpha->invert filter over the widened region", () => {
  ...
  const filterG = findEl(
    ...
    (e) =>
      e.tagName === "g" &&
      e.getAttribute("filter") === `url(#${filter.getAttribute("id")})`,
  );
...
test("compositeMask: a source shared by two masked nodes gets an independent copy per mask, with live transforms in each", () => {
  ...
  for (const c of circles)
    expect(c!.parentNode!.getAttribute("transform")).toBe(
      "matrix(1,0,0,1,10,0)",
    );
...
test("compositeFilter: wraps content in a CSS-filtered group; nests a matte inside", () => {
  ...
  const filtered = findEl(
    rootG,
    (e) => e.tagName === "g" && e.getAttribute("style") === "filter: blur(2px)",
  );
  ...
  const maskWrapper = findEl(
    filtered!,
    (e) => e.tagName === "g" && e.getAttribute("mask") === `url(#${maskId})`,
  );
...
test("rebuild on the same <svg>: exactly one defs + one root <g>, no duplicate ids", () => {
  ...
  const ids = findAll(svg, (e) => e.getAttribute("id") !== null).map(
    (e) => e.getAttribute("id")!,
  );
...
function bracketRecorder(): Renderer & { evs: Ev[]; keysInOrder: string[] } {
  ...
  return {
    evs: [],
    keysInOrder: [],
    beginNode(key: string) {
      this.evs.push({ t: "begin", key });
    ...
    drawImage: noop,
    clip: noop,
    compositeMask: (_m, drawContent, drawMask) => {
      drawContent();
      drawMask();
    },
    setFill: noop,
    setFillGradient: noop,
    setStroke: noop,
    setStrokeGradient: noop,
    setStrokeLineCap: noop,
    setStrokeLineJoin: noop,
    setStrokeMiterLimit: noop,
    setTrim: noop,
    setDash: noop,
    setFillRule: noop,
    setPaintOrder: noop,
...
test("render walk brackets nodes with nested, stable keys across seeks", () => {
  ...
  expect(shape).toEqual(["+n0", "+n1", "+n2", "-", "-", "+n3", "-", "-"]);
