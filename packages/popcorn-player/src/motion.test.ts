...
import { parsePath, buildMotionPath, samplePathAt } from './scene/path-parser';
...
import type { SceneNode, KeyframeData, AnimationInstance, CircleData } from './scene/types';
...
function makeAnim(partial: Partial<AnimationInstance>): AnimationInstance {
  return {
    name: 'a', duration: 100, timingFunction: 'linear', iterationCount: 1,
    direction: 'normal', delay: 0, fillMode: 'forwards', keyframes: [], ...partial,
  };
}
...
test('step-end: holds the departing value across the segment, then jumps (number + color)', () => {
  ...
  const kf: KeyframeData[] = [
    { offset: 0, properties: { r: 0, fill: 'rgb(0, 0, 0)' }, easing: 'step-end' },
...
test('hit-test agrees with the render matrix for a node mid-path', () => {
  ...
  rect.shapeData = { type: 'rect', x: 0, y: 0, width: 20, height: 20, rx: 0, ry: 0 };
...
test('negative delay: value at t=0 equals a zero-delay animation at t=|delay|', () => {
  ...
  const a = circleNode();
  a.animations = [makeAnim({ delay: -450, duration: 100, iterationCount: Infinity, keyframes: kf })];
  resetNodeToBase(a);
  sched.sampleNode(a, 0);

  const b = circleNode();
  b.animations = [makeAnim({ delay: 0, duration: 100, iterationCount: Infinity, keyframes: kf })];
  resetNodeToBase(b);
...
test('negative delay: iteration accounting includes the skipped part (alternate)', () => {
  ...
  node.animations = [makeAnim({
    delay: -125, duration: 100, iterationCount: Infinity, direction: 'alternate', keyframes: kf,
  })];
  resetNodeToBase(node);
...
test('negative delay: backwards fill never shows (animation already active at t=0)', () => {
  ...
  node.base = snapshotNode(node);
  node.animations = [makeAnim({
    delay: -50, duration: 100, fillMode: 'backwards', keyframes: [
      { offset: 0, properties: { r: 0 } },
      { offset: 1, properties: { r: 100 } },
    ],
  })];
