import type { Color, GradientData, TrimDescriptor, Matrix3x3 } from './types';
import type { StrokeLineCap, StrokeLineJoin, FillRule, PaintOrder } from '../scene/types';
import { colorToCSS, multiplyMatrices, invertMatrix, IDENTITY_MATRIX } from './types';
...
export abstract class PaintStateRenderer {
  ...
  protected pushCtm(): void { this.ctmStack.push(this.ctm); }
  protected popCtm(): void { this.ctm = this.ctmStack.pop() ?? IDENTITY_MATRIX; }
  protected concatCtm(m: Matrix3x3): void { this.ctm = multiplyMatrices(this.ctm, m); }
  ...
  protected resetCtm(): void { this.ctm = IDENTITY_MATRIX; this.ctmStack.length = 0; }
}
