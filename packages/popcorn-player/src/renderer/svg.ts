...
export function pathToD(commands: PathCommand[]): string {
  ...
  for (const c of commands) {
    switch (c.type) {
      ...
      case 'H': d += `H${c.x}`; break;
      case 'V': d += `V${c.y}`; break;
      case 'C': d += `C${c.x1} ${c.y1} ${c.x2} ${c.y2} ${c.x} ${c.y}`; break;
      case 'S': d += `S${c.x2} ${c.y2} ${c.x} ${c.y}`; break;
      case 'Q': d += `Q${c.x1} ${c.y1} ${c.x} ${c.y}`; break;
      case 'T': d += `T${c.x} ${c.y}`; break;
      case 'A': d += `A${c.rx} ${c.ry} ${c.angle} ${c.largeArc ? 1 : 0} ${c.sweep ? 1 : 0} ${c.x} ${c.y}`; break;
      case 'Z': d += 'Z'; break;
    }
  }
  return d;
...
export function diffAttr(
  ...
  if (value === null) {
    if (prev !== undefined) { el.removeAttribute(name); cache.delete(name); }
  } else if (prev !== value) {
 ...
 */
export function maskModePlumbing(mode: MaskMode): { maskType: 'alpha' | 'luminance'; filter: MaskFilterPrimitive[] | null } {
  switch (mode) {
    case 'alpha': return { maskType: 'alpha', filter: null };
    case 'luminance': return { maskType: 'luminance', filter: null };
    case 'alpha-invert': return { maskType: 'alpha', filter: ['invertAlpha'] };
    case 'luminance-invert': return { maskType: 'alpha', filter: ['luminanceToAlpha', 'invertAlpha'] };
  }
}
 ...
 */
export function deviceRegionInUserSpace(inv: Matrix3x3, w: number, h: number): Bounds {
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  for (const [x, y] of [[0, 0], [w, 0], [w, h], [0, h]]) {
    const ux = inv[0] * x + inv[1] * y + inv[2];
    const uy = inv[3] * x + inv[4] * y + inv[5];
    if (ux < minX) minX = ux; if (uy < minY) minY = uy;
    if (ux > maxX) maxX = ux; if (uy > maxY) maxY = uy;
  }
...
interface DefEntry { el: SVGElement; lastFrame: number; }
interface GradEntry extends DefEntry { tag: string; sig: string; }
interface ClipEntry extends DefEntry { sig: string; }
...
export class SVGRenderer extends PaintStateRenderer implements Renderer {
  ...
  constructor(svg: SVGSVGElement) {
    ...
    this.root = {
      key: '__root__', g: rootG, worldAtOpen: IDENTITY_MATRIX,
      shapes: [], drawCursor: 0, childKeys: [], prevChildKeys: [], clipThisFrame: false,
      maskCursor: 0, filterCursor: 0, lastFrame: 0,
    };
  ...
  endFrame(): void {
    ...
    for (const [key, e] of this.groups) {
      if (e.lastFrame !== this.frame) { e.g.remove(); this.groups.delete(key); }
    }
    for (const [id, e] of this.gradients) {
      if (e.lastFrame !== this.frame) { e.el.remove(); this.gradients.delete(id); }
    }
    for (const [id, e] of this.clips) {
      if (e.lastFrame !== this.frame) { e.el.remove(); this.clips.delete(id); }
    }
  ...
  beginNode(key: string): void {
    ...
    if (!e) {
      ...
      e = { key: fullKey, g, worldAtOpen: this.ctm, shapes: [], drawCursor: 0, childKeys: [], prevChildKeys: [], clipThisFrame: false, maskCursor: 0, filterCursor: 0, lastFrame: this.frame };
  ...
  drawEllipse(cx: number, cy: number, rx: number, ry: number): void {
    ...
    this.applyPaint(el, { x: cx - rx, y: cy - ry, width: rx * 2, height: ry * 2 });
  }
  ...
  drawText(text: string, x: number, y: number, fontSize: number, fontFamily: string, fontWeight: string, anchor: TextAnchor): void {
    ...
    if (this.textCache.get(el) !== text) { el.textContent = text; this.textCache.set(el, text); }
  ...
  drawImage(src: string, x: number, y: number, w: number, h: number): void {
    ...
    this.flushGroupTransform();
    this.setAttr(el, 'opacity', this.opacity === 1 ? null : String(this.opacity));
  }
  ...
  compositeMask(mode: MaskMode, drawContent: () => void, drawMask: () => void): void {
    ...
    const region = deviceRegionInUserSpace(invertMatrix(pw), this.width, this.height);
    ...
    if (m.modeSig !== modeSig) {
      ...
      if (plumb.filter) {
        ...
        if (m.filterEl) { m.filterEl.remove(); m.filterEl = null; }
      }
      m.modeSig = modeSig;
    }
    ...
    const c = m.container;
    c.worldAtOpen = pw;
    c.drawCursor = 0; c.childKeys = []; c.clipThisFrame = false;
    c.maskCursor = 0; c.filterCursor = 0; c.lastFrame = this.frame;
  ...
  private top(): GroupEntry { return this.groupStack[this.groupStack.length - 1]; }

  private setAttr(el: Element, name: string, value: string | null): void {
    let c = this.attrCache.get(el);
    if (!c) { c = new Map(); this.attrCache.set(el, c); }
  ...
  private applyPaint(el: SVGElement, bounds: Bounds): void {
    ...
    this.setAttr(el, 'paint-order', this.paintOrder === 'stroke' ? 'stroke' : null);
  ...
  private ensureGradient(id: string, g: GradientData, bounds: Bounds): void {
    ...
    if (e && e.tag !== r.tag) { e.el.remove(); this.gradients.delete(id); e = undefined; }
    if (!e) {
    ...
    if (e.sig !== sig) {
      for (const [k, v] of Object.entries(r.coords)) e.el.setAttribute(k, String(v));
  ...
  private ensureClip(key: string, clip: ResolvedClip): void {
    ...
    if (e.sig !== sig) {
      ...
      if (this.fillRule === 'evenodd') shape.setAttribute('clip-rule', 'evenodd');
  ...
  private ensureMask(id: string, pw: Matrix3x3): MaskEntry {
    ...
    if (!m) {
      ...
      const container: GroupEntry = {
        key: `${id}$c`, g: containerG, worldAtOpen: pw,
        shapes: [], drawCursor: 0, childKeys: [], prevChildKeys: [], clipThisFrame: false,
        maskCursor: 0, filterCursor: 0, lastFrame: this.frame,
      };
      m = { maskEl, filterG, filterEl: null, container, modeSig: '', lastFrame: this.frame };
      this.masks.set(id, m);
    }
    return m;
  }
  ...
  private ensureMaskFilter(m: MaskEntry, filterId: string, prims: MaskFilterPrimitive[]): void {
    let f = m.filterEl;
    if (!f) {
  ...
  private ensureSynthetic(key: string, parentG: SVGGElement, worldAtOpen: Matrix3x3): GroupEntry {
    let e = this.groups.get(key);
    if (!e) {
      const g = document.createElementNS(SVGNS, 'g') as SVGGElement;
      parentG.appendChild(g);
      e = {
        key, g, worldAtOpen, shapes: [], drawCursor: 0, childKeys: [], prevChildKeys: [],
        clipThisFrame: false, maskCursor: 0, filterCursor: 0, lastFrame: this.frame,
      };
    ...
    e.drawCursor = 0; e.childKeys = []; e.clipThisFrame = false;
    e.maskCursor = 0; e.filterCursor = 0; e.lastFrame = this.frame;
    return e;
  }
  ...
  private reconcileGroup(e: GroupEntry): void {
    this.setAttr(e.g, 'clip-path', e.clipThisFrame ? `url(#${this.idp}clip_${e.key})` : null);

    for (let i = e.shapes.length - 1; i >= e.drawCursor; i--) {
      const el = e.shapes[i];
      if (el) { el.remove(); this.attrCache.delete(el); }
      e.shapes.pop();
    }
