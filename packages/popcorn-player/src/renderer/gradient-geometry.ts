...
export interface ResolvedRadialGradient {
  type: 'radial';
  cx: number; cy: number; r: number;   // outer circle
  fx: number; fy: number;              // focal = inner-circle centre, radius 0
  stops: { offset: number; color: string }[];
}
 ...
 */
export function resolveGradient(g: GradientData, b: PaintBox): ResolvedGradient {
  const cx = b.x + b.width / 2;
  const cy = b.y + b.height / 2;
  const stops = g.stops.map((s) => ({ offset: Math.max(0, Math.min(1, s.offset)), color: s.color }));

  if (g.type === 'linear-gradient') {
    if (g.from && g.to) {
      return { type: 'linear', x1: g.from.x, y1: g.from.y, x2: g.to.x, y2: g.to.y, stops };
    }
  ...
  if (g.at && g.radius != null) {
    ...
    return { type: 'radial', cx: g.at.x, cy: g.at.y, r: g.radius, fx: f.x, fy: f.y, stops };
  }
