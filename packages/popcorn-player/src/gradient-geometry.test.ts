...
import type { GradientData, LinearGradientData, RadialGradientData } from './renderer/types';
...
function mockGradCanvas() {
  ...
  const ctx: any = {
    ...
    createLinearGradient(...args: number[]) { calls.push({ op: 'linear', args }); return grad; },
    createRadialGradient(...args: number[]) { calls.push({ op: 'radial', args }); return grad; },
  };
  return { getContext: () => ctx, calls } as any;
}

const realize = (g: GradientData, b: { x: number; y: number; width: number; height: number }) => {
...
test('explicit linear renders point-to-point, ignoring the bbox', () => {
  const g: LinearGradientData = {
    type: 'linear-gradient', angle: 180, stops: [],
    from: { x: -20, y: -500 }, to: { x: -19, y: 545 },
  };
...
test('explicit radial renders createRadialGradient(cx,cy,0,cx,cy,r)', () => {
  const g: RadialGradientData = {
    type: 'radial-gradient', stops: [], radius: 271.5, at: { x: 5, y: 6 },
  };
  const call = realize(g, { x: 0, y: 0, width: 10, height: 10 });
...
test('radial focal moves the INNER circle center only', () => {
  const g: RadialGradientData = {
    type: 'radial-gradient', stops: [], radius: 100, at: { x: 10, y: 20 }, focal: { x: 40, y: 30 },
  };
  const call = realize(g, { x: 0, y: 0, width: 10, height: 10 });
...
test('interpolateGradient lerps linear from/to endpoints', () => {
  const a: LinearGradientData = { type: 'linear-gradient', angle: 0, stops: [{ offset: 0, color: '#000' }], from: { x: 0, y: 0 }, to: { x: 10, y: 0 } };
  const b: LinearGradientData = { type: 'linear-gradient', angle: 0, stops: [{ offset: 0, color: '#000' }], from: { x: 10, y: 20 }, to: { x: 30, y: 40 } };
...
test('interpolateGradient lerps radial radius/at/focal', () => {
  const a: RadialGradientData = { type: 'radial-gradient', stops: [{ offset: 0, color: '#000' }], radius: 100, at: { x: 0, y: 0 }, focal: { x: 0, y: 0 } };
  const b: RadialGradientData = { type: 'radial-gradient', stops: [{ offset: 0, color: '#000' }], radius: 200, at: { x: 10, y: 10 }, focal: { x: 20, y: 20 } };
...
test('explicit and bbox-derived gradients are incompatible (step, no lerp)', () => {
  const a: LinearGradientData = { type: 'linear-gradient', angle: 0, stops: [{ offset: 0, color: '#000' }], from: { x: 0, y: 0 }, to: { x: 1, y: 0 } };
  const b: LinearGradientData = { type: 'linear-gradient', angle: 0, stops: [{ offset: 0, color: '#000' }] };
