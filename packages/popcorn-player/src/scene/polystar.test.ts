...
test("4-point star: alternating outer/inner vertices, first point up", () => {
  ...
  expect(rounded(cmds[1])).toEqual({
    type: "L",
    x: Math.round(d * 1000) / 1000,
    y: Math.round(-d * 1000) / 1000,
  });
...
test("roundness > 0 emits cubic beziers instead of straight lines", () => {
  ...
  const round = polystarToCommands({
    ...base,
    type: "star",
    sides: 5,
    outerRoundness: 50,
