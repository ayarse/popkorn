...
import type { VariableDefinition } from '@popcorn/parser';
...
test('boolean variables resolve as booleans', () => {
  const r = createVariableResolver();
  r.setVariables([{ name: '--pressed', value: { type: 'keyword', value: 'false' } }]);
...
test('trigger fires true for one frame, then endFrame resets it', () => {
  const r = createVariableResolver();
  r.setVariables([{ name: '--tap', value: { type: 'keyword', value: 'trigger' } }]);
...
test('unknown input path falls back to 0', () => {
  ...
  const defs: VariableDefinition[] = [
    { name: '--x', value: { type: 'function', name: 'input', args: [{ type: 'keyword', value: 'bogus.path' }] } },
  ];
  r.setVariables(defs);
...
test('scroll.progress input reads the tracked value', () => {
  const r = createVariableResolver();
  r.setVariables([
    { name: '--p', value: { type: 'function', name: 'input', args: [{ type: 'keyword', value: 'scroll.progress' }] } },
  ]);
