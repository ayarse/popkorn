...
// The lamp bulb outline from examples/popcorn/13-state-machine--lamp.css.
const BULB_D =
...
// through). Emulates PopcornView onTouch/onTouchEnd: set cursor + pressed latch,
...
test("machine click() on a PATH-shaped node fires from a device tap", () => {
  ...
  try {
    ...
    cursor.y = 100;
    cursor.isDown = true;
    cursor.pressed = true; // set by PopcornView.onTouch
