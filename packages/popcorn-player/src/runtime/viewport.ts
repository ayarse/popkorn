...
export const IDENTITY_VIEWPORT: Viewport = { scaleX: 1, scaleY: 1, offsetX: 0, offsetY: 0 };
...
export function computeViewport(
  ...
  switch (fit) {
    ...
    case 'cover': {
      ...
      return { scaleX: s, scaleY: s, offsetX: (dw - sceneW * s) / 2, offsetY: (dh - sceneH * s) / 2 };
    }
    case 'contain':
    default: {
...
export function deviceToScene(vp: Viewport, deviceX: number, deviceY: number): { x: number; y: number } {
  return { x: (deviceX - vp.offsetX) / vp.scaleX, y: (deviceY - vp.offsetY) / vp.scaleY };
}
