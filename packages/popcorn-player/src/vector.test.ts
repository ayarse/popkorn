...
test('builder: linear-gradient defaults angle to 180deg (to bottom)', () => {
  const node = firstNode('#a { type: rect; fill: linear-gradient(#fff 0%, #000 100%); }');
  expect((node.fillGradient as LinearGradientData).angle).toBe(180);
});

test('builder: radial-gradient parses stops (no angle)', () => {
  const node = firstNode('#b { type: circle; r: 50px; fill: radial-gradient(#ffffff 0%, #000000 100%); }');
...
test('builder: gradients are allowed on stroke', () => {
  const node = firstNode('#a { type: rect; stroke: linear-gradient(45deg, #000 0%, #fff 100%); }');
  expect(node.stroke).toBeNull();
  expect((node.strokeGradient as LinearGradientData).type).toBe('linear-gradient');
});
...
test('builder: clip-path circle() parses radius and center', () => {
  const node = firstNode('#g { type: group; clip-path: circle(50px at 100px 120px); }');
  expect(node.clipPath).toEqual({ type: 'circle', r: 50, x: 100, y: 120 });
});

test('builder: clip-path inset() parses four insets', () => {
  const node = firstNode('#g { type: group; clip-path: inset(10px 20px 30px 40px); }');
  expect(node.clipPath).toEqual({ type: 'inset', top: 10, right: 20, bottom: 30, left: 40 });
});

test('builder: clip-path path() parses commands', () => {
  const node = firstNode("#g { type: group; clip-path: path('M0 0 L100 0 L0 100 Z'); }");
...
test.skipIf(!hasPath2D)('hitTest: path clip rejects points outside the path', () => {
  const group = createSceneNode('g', 'group');
  group.clipPath = { type: 'path', commands: [
    { type: 'M', x: 0, y: 0 },
    { type: 'L', x: 100, y: 0 },
    { type: 'L', x: 0, y: 100 },
    { type: 'Z' },
