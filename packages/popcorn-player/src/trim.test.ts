...
test('computePathLength: straight polyline is exact', () => {
  ...
  expect(computePathLength(parsePath('M0 0 L100 0 L100 100'))).toBeCloseTo(200, 6);
});
...
test('computePathLength: collinear cubic measures its chord', () => {
  ...
  expect(computePathLength(parsePath('M0 0 C33 0 66 0 100 0'))).toBeCloseTo(100, 6);
});
...
test('computePathLength: quarter-circle cubic approximates the true arc length', () => {
  ...
  const len = computePathLength(parsePath('M100 0 C100 55.2285 55.2285 100 0 100'));
  expect(len).toBeCloseTo(157.08, 0); // within 0.5
});

test('shapeOutlineLength: circle is 2*pi*r', () => {
  expect(shapeOutlineLength({ type: 'circle', cx: 0, cy: 0, r: 50 })).toBeCloseTo(2 * Math.PI * 50, 6);
});

test('shapeOutlineLength: sharp rect perimeter is 2*(w+h)', () => {
  expect(shapeOutlineLength({ type: 'rect', x: 0, y: 0, width: 200, height: 100, rx: 0, ry: 0 }))
    .toBeCloseTo(600, 6);
});
...
test('shapeOutlineLength: rounded rect = straight edges + one full corner ellipse', () => {
  ...
  const len = shapeOutlineLength({ type: 'rect', x: 0, y: 0, width: 200, height: 100, rx: 10, ry: 10 });
...
test('builder: trim percentages normalize to 0..1', () => {
  const node = firstNode('#p { type: circle; r: 50px; trim-start: 25%; trim-end: 75%; trim-offset: 10%; }');
...
test('builder: stroke-linecap keywords', () => {
  for (const cap of ['butt', 'round', 'square'] as const) {
    const node = firstNode(`#p { type: circle; r: 50px; stroke-linecap: ${cap}; }`);
...
test('computeTrim: empty window strokes nothing', () => {
  ...
  n.trimStart = 0.6;
  n.trimEnd = 0.4;
  expect(computeTrim(n)).toEqual({ visible: false, dashArray: [], dashOffset: 0 });
});
