...
test('viewBox sets stage size; zero min emits no root translate', () => {
  const { css } = conv(`<svg viewBox="0 0 320 240"><rect width="10" height="10"/></svg>`);
...
test('non-zero viewBox min bakes a root translate group', () => {
  const { css } = conv(`<svg viewBox="10 20 100 80"><rect width="10" height="10"/></svg>`);
...
test('falls back to width/height attrs when no viewBox', () => {
  const { css } = conv(`<svg width="64" height="48"><rect width="10" height="10"/></svg>`);
...
test('rect maps x/y/width/height and mirrors rx-only to ry', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><rect x="5" y="6" width="30" height="20" rx="4" fill="#123456"/></svg>`);
...
test('circle and ellipse map to native geometry props', () => {
  const c = conv(`<svg viewBox="0 0 100 100"><circle cx="10" cy="20" r="5"/></svg>`).css;
  expect(block(c, 'circle1')).toContain('type: circle');
  expect(block(c, 'circle1')).toMatch(/cx: 10px[\s\S]*cy: 20px[\s\S]*r: 5px/);
  const e = conv(`<svg viewBox="0 0 100 100"><ellipse cx="10" cy="20" rx="8" ry="4"/></svg>`).css;
...
test('path d is copied byte-for-byte (whitespace normalized)', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><path d="M10 10 L50 50 Z"/></svg>`);
  expect(block(css, 'path1')).toContain(`d: 'M10 10 L50 50 Z'`);
});

test('line/polyline/polygon synthesize a path d (polygon closes with Z)', () => {
  const c = conv(`<svg viewBox="0 0 10 10"><line x1="0" y1="0" x2="10" y2="10"/><polyline points="0,0 1,1 2,0"/><polygon points="0,0 5,0 5,5"/></svg>`).css;
...
test('fill-rule evenodd is emitted', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><path d="M0 0 H10 V10 H0 Z" fill-rule="evenodd"/></svg>`);
  expect(block(css, 'path1')).toContain('fill-rule: evenodd');
});

test('fill="none" emits fill: none; absent fill defaults to black', () => {
  const none = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" fill="none"/></svg>`).css;
  expect(block(none, 'rect1')).toContain('fill: none');
  const def = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5"/></svg>`).css;
  expect(block(def, 'rect1')).toContain('fill: #000000');
});

// --- stroke -----------------------------------------------------------------

test('stroke props map with matching Popcorn names', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><path d="M0 0 L10 10" stroke="#f0f" stroke-width="2" stroke-linecap="round" stroke-linejoin="bevel" stroke-miterlimit="3"/></svg>`);
...
test('stroke-dasharray / dashoffset map to px lengths', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><path d="M0 0 L10 10" stroke="black" stroke-dasharray="4 2" stroke-dashoffset="1"/></svg>`);
...
test('fill-opacity / stroke-opacity fold into rgba alpha', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" fill="#ff0000" fill-opacity="0.5" stroke="#00ff00" stroke-opacity="0.25"/></svg>`);
...
test('named colors, rgb(), hsl() and currentColor all normalize', () => {
  const named = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" fill="rebeccapurple"/></svg>`);
  ...
  const rgb = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" fill="rgb(10,20,30)"/></svg>`).css;
  expect(block(rgb, 'rect1')).toContain('fill: #0a141e');
  const hsl = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" fill="hsl(0,100%,50%)"/></svg>`).css;
  expect(block(hsl, 'rect1')).toContain('fill: #ff0000');
});

test('currentColor resolves to the inherited color', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><g color="#ff0000"><rect width="5" height="5" fill="currentColor"/></g></svg>`);
...
test('cascade precedence: presentation < <style> < inline', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><style>.a{fill:green}</style><rect class="a" fill="red" style="fill:blue" width="5" height="5"/></svg>`);
  expect(block(css, 'rect1')).toContain('fill: #0000ff'); // inline blue wins
});

test('specificity: #id rule beats .class rule', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><style>.a{fill:green} #r{fill:red}</style><rect id="r" class="a" width="5" height="5"/></svg>`);
  expect(block(css, 'r')).toContain('fill: #ff0000');
});

test('descendant selector matches through ancestors', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><style>g rect{fill:teal}</style><g><rect width="5" height="5"/></g></svg>`);
  expect(css).toContain('fill: #008080');
});

test('exotic combinator selectors warn and are ignored', () => {
  const { warnings } = conv(`<svg viewBox="0 0 10 10"><style>g > rect{fill:teal}</style><g><rect width="5" height="5"/></g></svg>`);
  expect(warnings.some((w) => w.includes('exotic'))).toBe(true);
});

test('fill inherits down a group; opacity does not', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><g fill="#abcdef" opacity="0.5"><rect width="5" height="5"/></g></svg>`);
...
test('translate/scale decompose onto the group', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><g transform="translate(10,20) scale(2)"><rect width="5" height="5"/></g></svg>`);
  expect(block(css, 'g2')).toContain('transform: translate(10px, 20px) scale(2)');
});

test('rotate(a cx cy) decomposes to translate + rotate', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><g transform="rotate(30 60 60)"><rect x="20" y="20" width="5" height="5"/></g></svg>`);
  const b = block(css, 'g2');
  expect(b).toMatch(/transform: translate\([^)]*\) rotate\(30deg\)/);
});

test('shear-free matrix() decomposes; no bake', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><g transform="matrix(1 0 0 1 40 40)"><rect width="5" height="5"/></g></svg>`);
...
test('sheared transform bakes geometry into a transformed path + does not emit a transform', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><g transform="skewX(-18)"><rect x="20" y="90" width="80" height="30" fill="#f00"/></g></svg>`);
...
test('<use> deep-clones the target with an x/y translate', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><defs><circle id="c" cx="0" cy="0" r="5" fill="#f00"/></defs><use href="#c" x="20" y="30"/></svg>`);
...
test('<symbol> viewBox contributes a viewport scale on <use>', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><symbol id="s" viewBox="0 0 10 10"><rect width="10" height="10" fill="teal"/></symbol><use href="#s" x="10" y="10" width="20" height="20"/></svg>`);
  expect(css).toMatch(/transform: translate\(10px, 10px\) scale\(2\)/);
});

test('<use> cycle is guarded with a warning', () => {
  const { warnings } = conv(`<svg viewBox="0 0 10 10"><g id="a"><use href="#a"/></g><use href="#a"/></svg>`);
...
test('userSpaceOnUse linear gradient uses raw coordinates', () => {
  const { css } = conv(`<svg viewBox="0 0 100 100"><defs><linearGradient id="g" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="100" y2="0"><stop offset="0" stop-color="#f00"/><stop offset="1" stop-color="#00f"/></linearGradient></defs><rect width="100" height="100" fill="url(#g)"/></svg>`);
  expect(block(css, 'rect1')).toContain('linear-gradient(from 0px 0px to 100px 0px, #ff0000 0%, #0000ff 100%)');
});

test('radial gradient maps center/radius/focal; stop-opacity 0 folds to rgba a=0', () => {
  const { css } = conv(`<svg viewBox="0 0 200 120"><defs><radialGradient id="g" cx="0.5" cy="0.5" r="0.5" fx="0.35" fy="0.35"><stop offset="0" stop-color="#fef08a"/><stop offset="1" stop-color="#f97316" stop-opacity="0"/></radialGradient></defs><circle cx="150" cy="40" r="34" fill="url(#g)"/></svg>`);
  const b = block(css, 'circle1');
  expect(b).toContain('radial-gradient(circle 34px at 150px 40px from 139.8px 29.8px');
...
test('spreadMethod reflect/repeat warns (pad only)', () => {
  const { warnings } = conv(`<svg viewBox="0 0 100 100"><defs><linearGradient id="g" spreadMethod="reflect"><stop offset="0" stop-color="#000"/><stop offset="1" stop-color="#fff"/></linearGradient></defs><rect width="100" height="100" fill="url(#g)"/></svg>`);
...
test('clipPath with a single circle child -> circle()', () => {
  const { css } = conv(`<svg viewBox="0 0 200 200"><defs><clipPath id="c"><circle cx="100" cy="100" r="80"/></clipPath></defs><g clip-path="url(#c)"><rect width="200" height="200"/></g></svg>`);
  expect(block(css, 'g2')).toContain('clip-path: circle(80px at 100px 100px)');
});

test('clipPath with multiple shapes -> union of path()', () => {
  const { css } = conv(`<svg viewBox="0 0 200 200"><defs><clipPath id="c"><rect x="0" y="0" width="60" height="60"/><rect x="80" y="80" width="60" height="60"/></clipPath></defs><g clip-path="url(#c)"><rect width="200" height="200"/></g></svg>`);
...
test('mask emits a matte reference + a hoisted luminance source node', () => {
  const { css } = conv(`<svg viewBox="0 0 200 200"><defs><mask id="m"><rect width="200" height="200" fill="#fff"/></mask></defs><g mask="url(#m)"><rect width="200" height="200" fill="#22d3ee"/></g></svg>`);
...
test('single feGaussianBlur -> filter: blur()', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><defs><filter id="b"><feGaussianBlur stdDeviation="3"/></filter></defs><circle cx="5" cy="5" r="4" filter="url(#b)"/></svg>`);
  expect(block(css, 'circle1')).toContain('filter: blur(3px)');
});

test('feDropShadow -> filter: drop-shadow()', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><defs><filter id="d"><feDropShadow dx="2" dy="2" stdDeviation="1" flood-color="#333" flood-opacity="0.5"/></filter></defs><rect width="4" height="4" filter="url(#d)"/></svg>`);
  expect(block(css, 'rect1')).toContain('filter: drop-shadow(2px 2px 1px rgba(51, 51, 51, 0.5))');
});

test('unsupported multi-primitive filter warns and is skipped', () => {
  const { css, warnings } = conv(`<svg viewBox="0 0 10 10"><defs><filter id="f"><feGaussianBlur stdDeviation="1"/><feColorMatrix/></filter></defs><rect width="4" height="4" filter="url(#f)"/></svg>`);
...
test('text maps content/x/y/font/anchor; tspans flatten with a warning', () => {
  const { css, warnings } = conv(`<svg viewBox="0 0 100 100"><text x="10" y="20" font-size="14" font-family="Comic Sans" font-weight="bold" text-anchor="middle" fill="#000">Hi <tspan>there</tspan></text></svg>`);
...
test('image with a data URI maps to content: url()', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><image href="data:image/png;base64,AAAA" x="0" y="0" width="10" height="10"/></svg>`);
...
test('SMIL animation is skipped with a warning', () => {
  const { warnings } = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5"><animate attributeName="x" to="5"/></rect></svg>`);
  expect(warnings.some((w) => w.includes('SMIL'))).toBe(true);
});

test('<style> @keyframes + CSS animation are skipped with warnings', () => {
  const { css, warnings } = conv(`<svg viewBox="0 0 48 48"><style>@keyframes spin { to { transform: rotate(360deg); } } .s{animation: spin 1s linear infinite}</style><rect class="s" width="10" height="10"/></svg>`);
...
test('pattern / marker / foreignObject / textPath land in the blocked set', () => {
  const pat = conv(`<svg viewBox="0 0 10 10"><pattern id="p"/><rect width="5" height="5"/></svg>`);
  expect(pat.blocked).toContain('pattern');
  const mk = conv(`<svg viewBox="0 0 10 10"><marker id="m"/><rect width="5" height="5"/></svg>`);
  expect(mk.blocked).toContain('marker');
  const fo = conv(`<svg viewBox="0 0 10 10"><foreignObject/><rect width="5" height="5"/></svg>`);
  expect(fo.blocked).toContain('foreignObject');
});

test('display:none / visibility:hidden drop the node', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><rect width="5" height="5" display="none"/><rect width="4" height="4" fill="#0f0"/></svg>`);
...
test('SVG ids are preserved (sanitized); missing names get stable generated ids', () => {
  const { css } = conv(`<svg viewBox="0 0 10 10"><rect id="my-box" width="5" height="5"/><circle cx="1" cy="1" r="1"/></svg>`);
...
test('exported API matches the frozen contract', () => {
  ...
  const out = convertSvg(`<svg viewBox="0 0 10 10"><rect width="5" height="5"/></svg>`);
...
test('every examples/svg fixture converts to valid Popcorn CSS', () => {
