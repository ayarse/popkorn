...
test("identical images: zero deltas, hamming 0", () => {
  ...
  const { meanDelta, maxDelta, coverage } = regionDelta(img, same, {
    x: 0,
    y: 0,
    w: W,
    h: H,
  ...
  expect(
    hamming(
      aHash(img, { x: 0, y: 0, w: W, h: H }),
      aHash(same, { x: 0, y: 0, w: W, h: H }),
    ),
  ).toBe(0);
});
...
test("a moved square: the cell(s) it moved into rank worst, hashDist > 0", () => {
  ...
  const cols = 4,
    rows = 4; // 4x4 pixel cells over a 16x16 image
  ...
  const worstKeys = cells
    .filter((c) => c.hashDist === cells[0].hashDist)
    .map((c) => `${c.col},${c.row}`);
